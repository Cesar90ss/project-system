-- TODO LIST (FILE SYSTEM) --
--> Thursday
- (2h) Current directory support
    - Open and read in directories, first with one thread then with 2 (in user
      and kernel) --> 4 tests
    - test syscall change current directory and get current directory - ?

- Current directory support :
    - If we have an @space
        --> return currentDir of @space
    - otherwise
        --> cur dir of kernel thread
    
- Cur dir as a char which need to be concatenated inside expand filename
    --> take care of absolute path name
- two methods :
    - Thread::GetCurrentDirectory()
    - Thread::SetCurrentDirector(args)
- two syscalls :
    - GetCurrentDirectory
    - SetCurrentDirectory

--- TODO OF THE DAY ---
- Implement Open, Close, Read, Write syscalls (1 file support for now)
- Implement  syscalls // 2tests in user mlode (like kernel thread)
- Implements 2 methods
- Modify expandFileName
- Tests for absolute paths (user mode)
-----------------------

--> Friday
- (2h) Open files table
- (2h) Create/delete file directory concurrently
- (1h) Increase file size
- (1h30) Dynamic file size
    
--> Monday
- (1h30) Absolute path
- (4h) Multiple thread concurrency (reader/writer)

--> Tuesday
- (4h) Optimize I/O
- (4h) Improve file system robustness
-----------------------------
